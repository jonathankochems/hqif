  ...
  describe "Data.Qif" $ do
    it "should satisfy that displaying and parsing yields the identity on one transaction" $ do
      let _qif = Qif {accountType = Bank, transactions = [Transaction {_date = Date.fromGregorian 2018 4 3, _payee = "r\227\245\&5\191!", _memo = ";\226S\224:+>aC1ik", _amount = -1037.0022, _category = Just "D:RlR", _investmentAction = Nothing, _clearedStatus = NotCleared, _splits = []}]}
          _parsedQif = qifFromString $ displayQif _qif
      displayQif _parsedQif `shouldBe` displayQif _qif
      _parsedQif `shouldBe` _qif
    it "should satisfy that displaying and parsing yields the identity on serveral transactions" $ do
      let _qif = Qif { accountType = Invoice
...
transaction = do
   ...
   return Transaction{ _date = _date, _payee = description, _memo = text, _amount = balance
                     , _category = _category, _investmentAction = inv_act, _clearedStatus = clr_sta
                     , _splits = [] }
